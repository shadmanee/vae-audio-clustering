...
from utils.clustering import _save_k_vs_silhouette_plot, _save_silhouette_plot, _append_metrics_to_csv, _has_true_labels, compute_supervised_metrics, _score_for_model_selection, _preprocess_latents, _safe_internal_metrics, _cluster_size_summary, _save_dbscan_heatmap, _save_dbscan_coverage_plot, _save_dbscan_k_distance_plot, _save_cluster_scatter_2d, _save_truncated_dendrogram, _save_agglomerative_heatmap, _save_agglomerative_k_vs_silhouette
...
def run_KMeans(
    ...
    for k in k_range:
        ...
        if _has_true_labels(true_labels):
            metrics.update(compute_supervised_metrics(true_labels, labels))
        else:
            metrics.update({"ari": None, "nmi": None, "purity": None})

        row = {"k": k, **metrics}
        search_rows.append(row)

        score = _score_for_model_selection(metrics)
        if score > best_score:
            best_score = score
            best_model = km
            best_labels = labels
    ...
    search_df.to_csv(save_dir / "kmeans_search_results.csv", index=False)
    
    _save_k_vs_silhouette_plot(search_df, save_dir)

    if best_model is None:
        raise RuntimeError("KMeans search failed to produce a valid clustering.")

    _save_silhouette_plot(X, best_model, best_k, save_dir)

    final_metrics = {
        "optimal_k": best_k,
        "inertia": best_metrics["inertia"],
        "silhouette": best_metrics["silhouette"],
        "ch_index": best_metrics["ch_index"],
        "db_index": best_metrics["db_index"],
        "ari": best_metrics.get("ari"),
        "nmi": best_metrics.get("nmi"),
        "purity": best_metrics.get("purity"),
        "cluster_sizes": _cluster_size_summary(best_labels)
    }

    _append_metrics_to_csv(final_metrics, f"{model_type}_kmeans", root / config.RESULT_DIR / Path("clustering"))

    print("\n" + "=" * 24)
    print(f"KMeans best k      : {best_k}")
    print(f"Silhouette         : {final_metrics['silhouette']:.4f}" if final_metrics["silhouette"] is not None else "Silhouette         : None")
    print(f"CH Index           : {final_metrics['ch_index']:.4f}" if final_metrics["ch_index"] is not None else "CH Index           : None")
    print(f"DB Index           : {final_metrics['db_index']:.4f}" if final_metrics["db_index"] is not None else "DB Index           : None")
    print(f"Inertia            : {final_metrics['inertia']:.4f}")
    if _has_true_labels(true_labels):
        print(f"ARI                : {final_metrics['ari']:.4f}")
        print(f"NMI                : {final_metrics['nmi']:.4f}")
        print(f"Purity             : {final_metrics['purity']:.4f}")
    print(f"Cluster sizes      : {final_metrics['cluster_sizes']}")
    print("=" * 24 + "\n")

    return best_model, final_metrics, search_df
...
def run_DBSCAN(
    ...
    eps_quantiles=np.linspace(0.60, 0.98, 12),
    scale=True,
    l2_normalize=False,
    pca_var=0.95,
    root: Path = Path(".")
    ...
    for ms in min_samples_grid:
        _save_dbscan_k_distance_plot(X, ms, save_dir)

        nn = NearestNeighbors(n_neighbors=ms)
        nn.fit(X)
        distances, _ = nn.kneighbors(X)
        kth_dist = np.sort(distances[:, -1])
        candidate_eps = np.unique(np.quantile(kth_dist, eps_quantiles))
        ...
        for eps in candidate_eps:
            ...
            noise_ratio = 1.0 - (n_clustered / n_samples)
            coverage = 1.0 - noise_ratio

            unique_non_noise = np.unique(labels[non_noise_mask]) if n_clustered > 0 else []
            n_clusters = len(unique_non_noise)

            if n_clusters < 2:
                continue

            metrics = _safe_internal_metrics(X, labels, ignore_noise=True)
            metrics.update({
            ...
            if _has_true_labels(true_labels):
                ...
                supervised = compute_supervised_metrics(tl[non_noise_mask], labels[non_noise_mask])
                metrics.update(supervised)
            else:
                metrics.update({"ari": None, "nmi": None, "purity": None})

            search_rows.append(metrics.copy())

            score = _score_for_model_selection(metrics, noise_ratio=noise_ratio)
            if score > best_score:
                best_score = score
                best_model = model
                best_labels = labels
    ...
    search_df = pd.DataFrame(search_rows)
    search_df.to_csv(save_dir / "dbscan_search_results.csv", index=False)

    _save_dbscan_heatmap(search_df, save_dir)
    _save_dbscan_coverage_plot(search_df, save_dir)

    if best_model is None:
        print("Warning: DBSCAN failed to find a valid clustering.")
        empty_metrics = {
            "optimal_k": 0,
            "silhouette": None,
            "ch_index": None,
        ...
        _append_metrics_to_csv(
            empty_metrics,
            f"{model_type}_dbscan",
            root / config.RESULT_DIR / Path("clustering")
        )
        return None, empty_metrics, search_df

    _save_cluster_scatter_2d(
        X,
        best_labels,
        save_dir / "dbscan_clusters_2d.png",
        title=f"DBSCAN clusters ({model_type})"
    )

    final_metrics = {
        "optimal_k": best_metrics["optimal_k"],
        "eps": best_metrics["eps"],
        "min_samples": best_metrics["min_samples"],
        "silhouette": best_metrics["silhouette"],
        "ch_index": best_metrics["ch_index"],
        "db_index": best_metrics["db_index"],
        "n_noise": best_metrics["n_noise"],
    ...
    print("\n" + "=" * 24)
    print(f"DBSCAN eps         : {final_metrics['eps']:.4f}")
    print(f"min_samples        : {final_metrics['min_samples']}")
    print(f"Clusters found     : {final_metrics['optimal_k']}")
    print(f"Silhouette         : {final_metrics['silhouette']:.4f}" if final_metrics["silhouette"] is not None else "Silhouette         : None")
    print(f"CH Index           : {final_metrics['ch_index']:.4f}" if final_metrics["ch_index"] is not None else "CH Index           : None")
    print(f"DB Index           : {final_metrics['db_index']:.4f}" if final_metrics["db_index"] is not None else "DB Index           : None")
    print(f"Coverage           : {final_metrics['coverage']:.4f}")
    print(f"Noise ratio        : {final_metrics['noise_ratio']:.4f}")
    if _has_true_labels(true_labels):
        print(f"ARI                : {final_metrics['ari']:.4f}")
        print(f"NMI                : {final_metrics['nmi']:.4f}")
        print(f"Purity             : {final_metrics['purity']:.4f}")
...
def run_Agglomerative(
    ...
    if linkage_metric_grid is None:
        linkage_metric_grid = [
            ("ward", "euclidean"),
            ("complete", "euclidean"),
            ("average", "euclidean"),
    ...
    for linkage_name, metric_name in linkage_metric_grid:
        ...
        for k in k_range:
            try:
                ...
                print(f"Skipping linkage={linkage_name}, metric={metric_name}, k={k}: {e}")
                continue
            ...
            if score > best_score:
                ...
                best_labels = labels
                best_metrics = metrics.copy()
                best_cfg = (k, linkage_name, metric_name)

    search_df = pd.DataFrame(search_rows)
    search_df.to_csv(save_dir / "agglomerative_search_results.csv", index=False)

    _save_agglomerative_k_vs_silhouette(search_df, save_dir)
    _save_agglomerative_heatmap(search_df, save_dir)

    if best_model is None:
        raise RuntimeError("Agglomerative search failed to produce a valid clustering.")

    best_k, best_linkage, best_metric = best_cfg

    _save_cluster_scatter_2d(
        X,
        best_labels,
        save_dir / "agglomerative_clusters_2d.png",
        title=f"Agglomerative clusters ({model_type})"
    )

    _save_truncated_dendrogram(
        X,
    ...
    _append_metrics_to_csv(
        final_metrics,
        f"{model_type}_agglomerative",
        root / config.RESULT_DIR / Path("clustering")
    )

    print("\n" + "=" * 24)
    print(f"Agglomerative best : k={best_k}, linkage={best_linkage}, metric={best_metric}")
    print(f"Silhouette         : {final_metrics['silhouette']:.4f}" if final_metrics["silhouette"] is not None else "Silhouette         : None")
    print(f"CH Index           : {final_metrics['ch_index']:.4f}" if final_metrics["ch_index"] is not None else "CH Index           : None")
    print(f"DB Index           : {final_metrics['db_index']:.4f}" if final_metrics["db_index"] is not None else "DB Index           : None")
    if _has_true_labels(true_labels):
...
#     visualizer  = KElbowVisualizer(elbow_model, k=(2, 12), timings=True, force_model=True)
...
#     if _has_true_labels(true_labels):
#         supervised = compute_supervised_metrics(true_labels[labels != -1], labels[labels != -1]) 
#         metrics.update(supervised)
