...
weight_path, cfg_path = save_model(model=cvae, model_name="hard_1_cvae_audio_only", cfg=cvae.config, root=root)

train_latents, _ = extract_latents(model=cvae, loader=train_loader, device=device)
test_latents,  _  = extract_latents(model=cvae, loader=test_loader,  device=device)

vae_latents = np.concatenate([train_latents, test_latents], axis=0)

vae_kmeans, vae_kmeans_metrics, vae_kmeans_df = run_KMeans(vae_latents, model_type="hard_1_cvae_audio_only", root=root)
vae_dbscan, vae_dbscan_metrics, vae_dbscan_df  = run_DBSCAN(vae_latents, model_type="hard_1_cvae_audio_only", root=root)
vae_agglomerative, vae_agglomerative_metrics, vae_agglomerative_df = run_Agglomerative(vae_latents, model_type="hard_1_cvae_audio_only", root=root)
